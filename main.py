...
def host_sorgula(ip_adresi: str, detay: bool = False) -> HostSonuc:
    ...
    parcalar = ip_adresi.split(".") # split(".") gelen stringi içinde yazan karakter nezlinde bölüp arada kalan değerler ile bir liste oluşturuyor.
    ...
    if len(parcalar) != 4 or not all(p.isdigit() and 0 <= int(p) <= 255 for p in parcalar): # all for daki her p için kontrol yapıyor, isdigit() -> sayımı
...
def tara(istek: TaramaIstegi) -> HostSonuc:
    parcalar = istek.ip_adresi.split(".")
